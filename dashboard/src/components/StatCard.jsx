...
export default function StatCard({ title, value, prefix = '', delay = 0, change = null }) {
    ...
    return (
        <motion.div
            ...
            className="flex flex-col justify-center items-center text-center rounded-xl border border-glass-border bg-glass-bg backdrop-blur-xl p-2 sm:p-3 min-h-[80px] sm:min-h-[95px] overflow-hidden group hover:bg-glass-bg-hover transition-colors duration-300 relative"
        >
            ...
            <div className="flex flex-col items-center gap-1 sm:gap-3 relative z-10 w-full">
                ...
                {hasChange && (
                    <div className={`inline-flex items-center gap-1 px-2 py-1 rounded-md ${changeBg}`}>
                        ...
                        <span className={`text-[10px] sm:text-xs font-semibold ${changeColor}`}>
                            {isNeutral ? '0.00' : Math.abs(change).toFixed(2)}%
                        </span>
                        <span className="text-[10px] text-text-muted ml-0.5">vs ant.</span>
                    </div>
                )}
            </div>
        </motion.div>
